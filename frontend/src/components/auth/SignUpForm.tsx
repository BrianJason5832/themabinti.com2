...
const SignUpForm = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      // Clean phoneNumber: ensure it starts with 2547 or 2541
      let cleanedPhoneNumber = values.phoneNumber.trim();
      // Remove all non-digit characters
      cleanedPhoneNumber = cleanedPhoneNumber.replace(/\D/g, '');
      // If starts with 0 and next is 7 or 1, replace with 254
      if (cleanedPhoneNumber.length === 10 && cleanedPhoneNumber.startsWith('0') && (cleanedPhoneNumber[1] === '7' || cleanedPhoneNumber[1] === '1')) {
        cleanedPhoneNumber = '254' + cleanedPhoneNumber.slice(1);
      }
      // If starts with 7 or 1 and is 9 digits, prepend 254
      if (cleanedPhoneNumber.length === 9 && (cleanedPhoneNumber[0] === '7' || cleanedPhoneNumber[0] === '1')) {
        cleanedPhoneNumber = '254' + cleanedPhoneNumber;
      }
      // If already starts with 254 and is 12 digits, keep as is
      // Otherwise, throw error
      if (!/^254[17][0-9]{8}$/.test(cleanedPhoneNumber)) {
        throw new Error('Phone number must start with 2547 or 2541 and be 12 digits');
      }
